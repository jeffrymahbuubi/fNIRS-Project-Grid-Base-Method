...
const commands = {
  ...
  restore: () => {
    ...
    // Archive session only if it has meaningful activity (edits > 0 or tasks > 0)
    // Unnamed sessions with no activity are discarded automatically
    const hasActivity = session.metrics.edits > 0 || session.metrics.tasks > 0;
    if (hasActivity) {
